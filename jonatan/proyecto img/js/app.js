...
function crearhtml() {
  ...
  tweets.forEach((tweet) => {
    ...
    const botonlike = createButton(tweet.like ? "fas fa-heart red" : "far fa-heart", () => toggleLike(tweet));
    const botonborrar = createButton("borrar-tweet", () => borrarTweet(tweet.id), "X");
    const botonlikee = createButton("far fa-thumbs-up", () => incrementarContador(tweet), `Like (${tweet.likes || 0})`);
...
function incrementarContador(tweet) {
